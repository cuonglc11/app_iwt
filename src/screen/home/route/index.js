...
const RouteListScreen = ({ navigation }) => {
    ...
    const renItem = ({ item }) => (
        <TouchableOpacity
            ...
            style={{ flexDirection: 'row', marginTop: 20, height: 40, backgroundColor: '#fff', borderBottomColor: "rgba(0, 0, 0, 0.13)", borderBottomWidth: 3 }}>
            <Text style={{ flex: 3, fontSize: 18, marginStart: 10, color: 'black', fontWeight: 'bold' }}>{item.name}</Text>
            <Text style={{ flex: 3, fontSize: 18 , color: item.title ? app_color : "#D3D3D3"  }}>{item.title ? "Đã kích hoạt" :"Chưa kích hoạt"}</Text>
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <CustomTop text={"Danh sách tuyến cáp"} onclick={() => {
                ...
                <View style={{ alignSelf: 'center', margin: 10, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', height: 50, width: '100%', marginTop: 10, backgroundColor: '#fff', borderRadius: 30 }}>
